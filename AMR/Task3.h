...
//Serial1.println("Modbus Task..");
 ...
 int S = 0 ;
  ...
  //  for ( int ID_Slave = 1 ; ID_Slave <  6; ID_Slave++)  // 6 --->1 ตัว
   //       {

    //  ID_Slave = 1 ;
              
             for (char i = 0; i < 40 ; i++) // 8 is  total Register
               {        
   ...
   //   unsigned long  randNumber1 = random(9234567890);          
     //    Mux1       = randNumber1;
        ...
        Power1     = (Data_Slave[1]);  //Active Power Consumtion
       Current1   = (Data_Slave[2])*0.01;  //Current L1  A
      Current2   = (Data_Slave[3])*0.01;  //Current L2  A   
      Current3   = (Data_Slave[4])*0.01;  //Current L3  A       
   ...
   //   unsigned long     randNumr2 = random(9234567890); 
        Mux2    = (Data_Slave[8]);
        Power2     = (Data_Slave[9]);  //Volt L1 V
       Current4   = (Data_Slave[10])/100;  //Current L1  A
        Current5   = (Data_Slave[11])/100;  //Current L2  A
         Current6   = (Data_Slave[12])/100;  //Current L13 A           
  ...
  //     unsigned long     randNumber3 = random(9234567890); 
         Mux3    = (Data_Slave[16]);
        Power3     = (Data_Slave[17]);  //Volt L1 V
       Current7   = (Data_Slave[18])/100;  //Current L1  A
        Current8   = (Data_Slave[19])/100;  //Current L1  A
       Current9   = (Data_Slave[20])/100;  //Current L1  A
...
//   unsigned long     randNumber4 = random(9234567890); 
         Mux4    = (Data_Slave[24]);
        Power4     = (Data_Slave[25]);  //Volt L1 V
       Current10   = (Data_Slave[26])/100;  //Current L1  A
       Current11   = (Data_Slave[27])/100;  //Current L1  A
       Current12   = (Data_Slave[28])/100;  //Current L1  A       
   ...
   //       unsigned long     randNumber5 = random(9234567890); 
           ...
           Mux5    = (Data_Slave[32]);
        Power5     = (Data_Slave[33]);  //Volt L1 V
       Current13   = (Data_Slave[34])/100;  //Current L1  A
        Current14   = (Data_Slave[35])/100;  //Current L1  A       
       Current15   = (Data_Slave[36])/100;  //Current L1  A
